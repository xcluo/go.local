...
class Consumer(object):
    ...
    def __init__(self, channel, topic, callback, exchange, queue, routing_key,
                 style='topic', msg_id=None):
        """根据style类型来决定exchang以及queue的生成类型"""
        self.channel = channel
        self.callback = callback
        if style == 'topic':
            self.exchange = kombu.entity.Exchange(name=exchange,
                                                  type='topic',
                                                  durable=True,
                                                  auto_delete=False)
            self.queue = kombu.entity.Queue(name=queue,
...
class Publisher(object):
    ...
    def __init__(self, channel, topic, exchange, routing_key,
                 style='topic', msg_id=None):
        self.channel = channel
        if style == 'topic':
            self.exchange = kombu.entity.Exchange(name=exchange,
...
def call(msg):
    ...
    3.同时需要在consumer处理完消息回发回来时接收到,
      这就需要生成一个临时的consumer(与msg_id密切相关)
    4.远程consumer,接受到消息发现是需要回发的(msg_id),
      需要生成一个publisher扔回消息
...
if __name__ == '__main__':
    ...
    conn.create_consumer('network', callback)
    conn.consume()
